...
3. **Review freshness and contradictions** — Check freshness status, stale reasons, and contradiction records before deciding how to refresh
4. **Update in place when canonical** — Refresh the page body and frontmatter when the page remains the right canonical destination
5. **Mark lifecycle state** — Mark pages `stale` or `superseded` when they no longer reflect current evidence cleanly
6. **Record the refresh** — Update `docs/wiki/index.md` if needed and append `docs/wiki/log.md`
